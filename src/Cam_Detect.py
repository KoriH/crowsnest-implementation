...
def check_proximity(person_boxes, suitcase_box, threshold=100):
    """Check if any person is within a certain distance of a suitcase."""
    x1_s, y1_s, x2_s, y2_s = suitcase_box
...
def annotate_frame(frame, x1, y1, x2, y2, tracker_id, class_id, unattended=False):
    ...
    if class_name in trash_classes:
        ...
        if unattended:
            label = f"unattended suitcase {tracker_id}"
            color = (0, 0, 255)  # Red color
        else:
            label = f"suitcase {tracker_id}"
            color = (0, 255, 0)  # Green color
...
while cap.isOpened():
    ...
    for tracker_id, box, class_id in zip(detections.tracker_id, detections.xyxy, detections.class_id):
        class_name = model.names[class_id].lower()
        # For unattended suitcase
        if class_name == 'suitcase':
            if not check_proximity(person_boxes, box):  # Check proximity of any person to the suitcase
                if tracker_id not in suitcase_tracker:  
                    suitcase_tracker[tracker_id] = time.time() # Start or update the timer for unattended suitcase
                elif time.time() - suitcase_tracker[tracker_id] > suitcase_alert_threshold:
                    frame = annotate_frame(frame, *box, tracker_id, class_id, unattended=True) # annotate
                    # Send email alert
                    if tracker_id not in alerted_suitcases: # avoid repeating alert
                        send_email("Alert: Unattended Suitcase", "Unattended suitcase detected")
                        alerted_suitcases.add(tracker_id)
                        log_event("Alert - Unattended Suitcase Detected")
                    continue
            else:
                # Person is close to the suitcase, reset the timer and remove from alert list
                if tracker_id in suitcase_tracker:
                    del suitcase_tracker[tracker_id]
                if tracker_id in alerted_suitcases:
                    alerted_suitcases.remove(tracker_id) 
